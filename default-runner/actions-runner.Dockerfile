...
RUN echo "export RUNNER_TOKEN=$(curl -L -X POST -H \"Accept: application/vnd.github+json\" \
     -H \"Authorization: Bearer $ACCESS_TOKEN\" -H \"X-GitHub-Api-Version: 2022-11-28\" \
     https://api.github.com/orgs/sliide/actions/runners/registration-token)" >> .env.sh
